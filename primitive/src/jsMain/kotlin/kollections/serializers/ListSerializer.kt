...
actual class ListSerializer<T> actual constructor(serializer: KSerializer<T>) : KSerializer<List<T>> {
    ...
    actual override fun deserialize(decoder: Decoder): List<T> = surrogate.deserialize(decoder).intoList()

    actual override fun serialize(encoder: Encoder, value: List<T>) = surrogate.serialize(encoder, value.toKList())
}